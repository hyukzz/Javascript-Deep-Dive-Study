...
1. const 키워드는 재할당이 불가하다. 하지만 객체를 변경하는 것은 가능하다.
2. ES6을 사용하는 일반적인 경우 const 키워드를 사용하고 재할당이 필요한 경우에는 let 키워드로 변경한다.
3. var 키워드는 함수 레벨 스코프이기 때문에 의도치 않은 상황이 발생할 수 있다. 때문에 let, const 키워드를 사용하는 편이 낫다.
