...
- 함수 정의와 동시에 호출되는 함수이다.

2. 재귀 함수

- 자기 자신을 호출하는 것을 수행하는 함수이다.
...
- 어떤 외부 상태에 의존하지도 않고 변경하지도 않다. 부수 효과가 없는 함수를 순수함수라고 한다.
- 외부 상태에 의존하거나 외부 상태를 변경한다. 부수 효과가 있는 함수를 비순수 함수라고 함

## 📔 기억하고 싶은 내용

1. 함수를 호출할 때 함수는 함수 이름으로 호출하는 것이 아니라 함수 객체를 가리키는 식별자로 호출된다.
2. 함수 이름은 함수 몸체 내에서만 참조할 수 있는 식별자다. 우리가 일반적인 경우처럼 호출해 사용할 수 있는 이유는 엔진이 암묵적으로 동일한 이름의 식별자를 생성해 함수의 주소를 할당하기 때문이다.
3. 그렇기 때문에 함수 선언문으로 만든 함수는 함수 호이스팅으로 함수문 위에서 호출이 가능하다.
