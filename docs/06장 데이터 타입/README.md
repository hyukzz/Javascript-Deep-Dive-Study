...
자바스크립트의 모든 값은 데이터 타입을 갖는다. 그렇다면 변수는 데이터 타입을 가질까?라는 의문을 가질 수 있다.

C나 자바 같은 `정적 타입(static/strong type)언어`는 변수를 선언할 때 변수에 할당할 수 있는 값의 종류, 즉 데이터 타입을 사전에 선언해야 한다. 이를 명시적 타입 선언이라 한다.
...
- 만약 타입 체크를 통과하지 못했다면 에러를 발생시키고 프로그램의 실행 자체를 막는다. 이를 통해 타입의 일관성을 강제함으로써 더욱 안정적인 코드의 구현을 통해 `런타임`에 발생하는 에러를 줄인다.
- 대표적인 정적 타입 언어로 C, C++, 자바, 코틀린, 고, 하스켈, 러스트, 스칼라 등이 있다.

자바스크립트는 정적 타입 언어와 다르게 변수를 선언할 때 타입을 선언하지 않는다. 다만 `var, let, const` 키워드를 사용해 변수를 선언할 뿐이다. 어떠한 값이라도 자유롭게 할당할 수 있다.

- typeof 연산자로 변수를 연산하면 변수의 데이터 타입을 반환한다. 정확히 말하면 변수의 데이터 타입을 반환하는 것이 아니라 변수에 할당된 값의 데이터 타입을 반환한 것이다.
- 정적 타입 언어에서 말하는 데이터 타입과 개념이 다른데 정적 타입 언어는 변수 선언 시점에 변수의 타입이 결정되고 변수의 타입을 변경할 수 없다.
- `자바스크립트의 변수는 선언이 아닌 할당에 의해 타입이 결정(타입 추론)된다. 그리고 재할당에 의해 변수의 타입은 언제든지 동적으로 변할 수 있다.` 이러한 특징을 `동적 타이핑(dynamic typing)`이라고 한다.
- 자바스크립트를 정적 타입 언어와 구별하기 위해 `동적 타입(dynamic/weak type)언어`라 한다.
- 대표적인 동적 타입 언어로는 자바스크립트, 파이썬, PHP, 루비, 리스프, 펄 등이 있다.

처음으로 돌아가서 변수는 타입을 가질까?라는 의문점에 대답을 하자면, 기본적으로 변수는 타입을 갖지 않는다. 하지만 값은 타입을 갖는다. 따라서 현재 변수에 할당되어 있는 값에 의해 변수의 타입이 동적으로 결정된다고 표현하는 것이 적절하다. 변수는 값에 묶여 있는 값에 대한 별명이기 때문이다.

### `6.10.2 동적 타입 언어와 변수`

동적 타입 언어는 변수에 어떤 데이터 타입의 값이라도 자유롭게 할당할 수 있다. 이러한 동적 타입 언어의 특징은 데이터 타입에 대해 무감각해질 정도로 편리하지만 편리함의 이면에는 위험도 도사리고 있다.

모든 소프트웨어 아키텍처에는 트레이드오프가 존재하며, 모든 애플리케이션에 적합한 은 탄환(해결책)은 없듯이 동적 타입 언어 또한 구조적인 단점이 있다.
...
이러한 이유로 안정적인 프로그램을 만들기 위해 변수를 사용하기 이전에 데이터 타입을 체크해야 하는 경우가 있는데 이는 매우 번거로울뿐더러 코드의 양도 증가한다. 따라서 변수를 사용할 때 주의 사항은 다음과 같다.
...
1. 데이터 타입은 값의 종류를 말하고, 자바스크립트의 모든 값은 데이터 타입을 갖는다. 원시 타입으로 `숫자, 문자열, 불리언, undefined, null, 심벌 타입`이 있고, 객체 타입으로 `객체, 함수, 배열 등`이 있다.
2. 자바스크립트의 문자열은 원시 타입이며, 변경 불가능한 값(immutable value)이다.
3. 템플릿 리터럴 내에서 표현식을 삽입하려면 ${}으로 표현식을 감싼다.
4. C언어 에서 선언은 컴파일러에게 식별자의 존재를 알리는 것이고, 정의는 실제로 컴파일러가 변수를 생성해서 식별자와 메모리 주소가 연결되는 것이다. ECMAScript사양에서는 변수는 선언한다라고 표현하고, 함수는 정의한다라고 표현한다.
5. 데이터 타입이 필요한 이유는 1번째로 값을 저장할 때 확보해야 하는 "메모리 공간의 크기"를 결정하기 위함이고, 2번째로 값을 참조할 때 한 번에 읽어 들여할 "메모리 공간의 크기"를 결정하기 위함이다. 마지막으로 메모리에서 읽어 들인 "2진수를 어떻게 해석"할지 결정하기 위함이다.
6. 자바스크립트는 동적 타입 언어이다. 다른 정적 타입 언어와는 다르게 변수를 선언할 때 할당할 수 있는 데이터 타입을 사전에 선언하지 않는다. 이로인해 유연성이 좋지만 신뢰성이 낮아 잦은 데이터 타입 오류를 발생시킬 가능성이 높다. 요즘엔 안정적인 프로그램을 제작하기 위해 타입스크립트를 사용해 데이터 타입을 정적 타입 언어와 같이 지정하고, 컴파일 시점에서 에러를 해결하는 방식을 사용한다.
