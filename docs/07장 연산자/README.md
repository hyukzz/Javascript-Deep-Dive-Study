...
- 조건식의 평가 결과가 불리언 값이 아니면 불리언 값으로 암묵적 타입 변환된다. 이때 조건식이 참이면 콜론(:) 앞의 두 번째 피연산자가 평가되어 반환되고, 거짓이면 콜론(:) 뒤의 세 번째 피연산자가 평가되어 반환된다.
...
- if...else 문은 표현식이 아니라 문이다. 따라서 값처럼 사용할 수 없다.
...
- `조건에 따라 어떤 값을 결정할 때는 if...else 문보다 삼항 조건 연산자 표현식을 사용하는 편이 유리하다`
...
```javascript
논리 연산자로 구성된 복잡한 표현식은 가독성이 좋지 않아 한눈에 이해하기 어려울 때 드 모르간의 법칙을 가독성 좋게 변환할 수 있다.

// 논리부정연산자가 괄호 안으로 오면서 논리합과 논리곱이 서로 교환된다.
!(x || y) === (!x && !y)
!(x && y) === (!x || !y)
```
...
쉼표(,) 연산자는 왼쪽 피연산자부터 차례대로 피연산자를 평가하고 마지막 피연산자의 평가가 끝나면 마지막 피연산자의 평가를 반환한다.
...
연산자 결합 순서란 연산자의 어느 쪽(좌항 or 우항)부터 평가를 수행할 것인지를 나타내는 순서를 말한다.

| 결합 순서    | 연산자                                                                               |
| ------------ | ------------------------------------------------------------------------------------ |
| 좌항 -> 우항 | +, -, /, %, <, <=, >, >=, &&, 백슬래시 두 개 , ., [], (), ??, ?.,in, instanceof      |
| 우항 -> 좌항 | ++, --, 할당 연산자(=,+=,-=,...), !x, +x, -x, ++x, --x, typeof, delete, ?, ... : ... |

## 📔 기억하고 싶은 내용

1. 피연산자는 `값`으로 평가 가능해야 한다.
2. `피연산자의 개수`에 따라 `이항 산술 연산자`와 `단항 산술 연산자`로 구분할 수 있다.
3. 문자열 연결 연산자를 사용할 때 암묵적 타입 변환이 일어날 수 있다.
4. 동등 비교 연산자는 먼저 암묵적 타입 변환을 통해 타입을 일치시킨 후 같은 값인지 비교한다.
5. 삼항 조건 연산자 표현식은 값처럼 사용할 수 있지만 if...else 문은 값처럼 사용할 수 없다. 그렇기 때문에 삼항 조건 연산자 표현식만 변수에 값을 할당할 수 없다.
6. 논리 부정(!) 연산자는 언제나 불리언 값을 반환한다.
7. 연산에는 우선순위가 있는데 괄호를 사용해 우선순위를 지정하는 편이 편리하다.
