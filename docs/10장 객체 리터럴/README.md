...
1. 객체는 변경가능한 값이다.
2. 함수도 값이기 때문에 프로퍼티 값으로 사용할 수 있다.
3. 객체는 프로퍼티의 집합이며, 프로퍼티는 키와 값으로 구성된다.
4. 식별자 네이밍 규칙을 따르지 않는다면 따옴표를 꼭 사용해야 한다.
5. 원시 타입은 단 하나의 값만 나타내지만 객체 타입은 다양한 타입의 값(원시 값 또는 다른 객체)을 하나의 단위로 구성한 복합적인 자료구조(data structure)다.
